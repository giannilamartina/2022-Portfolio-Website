...
var StarSky = /*#__PURE__*/function (_Component) {
  _inheritsLoose(StarSky, _Component);

  function StarSky(props) {
    var _this;
    ...
    _this.Sketch = function (p) {
      ...
      p.setup = function () {
        var cnv = p.createCanvas(_this.canvasRef.current.offsetWidth, _this.canvasRef.current.offsetHeight, p.P2D);
        cnv.style('position', 'absolute');
        cnv.style('top', 0);
        cnv.style('left', 0);
        cnv.style('z-index', -99);
        p.frameRate(_this.props.frameRate || 60);
        colorProp = _this.props.starColor;
        starSky = new _StarSky(_this.props.starCount);
        bgCol = getRGBObject(_this.props.backgroundColor);
      };

      p.draw = function () {
        p.background(bgCol.r, bgCol.g, bgCol.b);
        colorProp = _this.props.starColor;
        if (_this.state.resize) p.resize();
        starSky.render();

        if (_this.props.debugFPS) {
          var fps = p["int"](p._frameRate);
          p.fill(255);
          p.text(fps, 0, 10);
        }
      };
      ...
      p.resize = function () {
        p.resizeCanvas(_this.canvasRef.current.offsetWidth, _this.canvasRef.current.offsetHeight);
        console.log('Resize', _this.canvasRef.current.offsetWidth, _this.canvasRef.current.offsetHeight);
        p.starSky = null;
        p.starSky = new _StarSky(_this.props.starCount);
      ...
      var _StarSky = /*#__PURE__*/function () {
        function _StarSky(count) {
          count = count || 250;
        ...
        var _proto2 = _StarSky.prototype;

        _proto2.render = function render() {
          p.noStroke();
          var setColor = getRGBObject(colorProp, [255, 255, 255]);

          for (var i = 0; i < this.count; i++) {
            if (colorProp === 'rainbow') setColor = {
              r: this.rainbow[i][0],
              g: this.rainbow[i][1],
              b: this.rainbow[i][2]
            };
            p.fill(setColor.r, setColor.g, setColor.b, this.brightness[i]);
  ...
  var _proto = StarSky.prototype;

  _proto.componentDidMount = function componentDidMount() {
    this.canvas = new p5(this.Sketch, this.canvasRef.current);
    this.canvasRef.current.addEventListener('resize', this.setState({
      resize: true
    }));
  };

  _proto.componentWillUnmount = function componentWillUnmount() {
    this.canvasRef.current.removeEventListener('resize', this.setState({
      resize: true
    }));
  };

  _proto.render = function render() {
    var applyStyle = _extends({
      overflow: 'clip',
      position: 'relative'
    }, this.props.style);

    if (this.props.pageBackground) {
      applyStyle = _extends({}, applyStyle, {
        width: '100vw',
        height: '100vh',
        position: 'fixed',
        top: 0,
...
function getRGBObject(input, defaultCol) {
  ...
  if ((input === null || input === void 0 ? void 0 : input.length) === 3) output = {
      r: input[0],
      g: input[1],
      b: input[2]
    };
  ...
  if (typeof input === 'string') {
    var rgbCol = w3color(input).toRgb();
    output = {
      r: rgbCol.r,
      g: rgbCol.g,
      b: rgbCol.b
